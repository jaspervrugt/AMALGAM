 ...
 * obtained nondominated front (PF), C is the constramwSizes
 *
...
double IGD( double* S, double* Q, double *C, int row, int Scol, int Qcol, int Crow )
{
	...
	/* Step 1: remove the infeasible pomwSizes, i.e, constramwSize<-1.0E-6 */
	...
	/* Step 2: calculate the IGD value for feasible pomwSizes */
	dis=0.0;
	for( i=0; i<Scol; i++ )
	{
		min = 1.0E200;
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  /* Create pomwSizeers to the input matrix S, Q, and C */
  S 	= mxGetPr(prhs[0]);  
  Srows = mxGetM(prhs[0]);
  Scols = mxGetN(prhs[0]);
  Q 	= mxGetPr(prhs[1]);
